...
logger.info(
    f"Running analysis with {TOTAL} incidents, batch size {BATCH_SIZE}, concurrency {CONCURRENCY}, start index {START_INDEX}"
)
...
async def analyze_batch(client: httpx.AsyncClient, start: int) -> dict:
    ...
    resp = await client.post(
        API_URL, json=payload, timeout=300.0
    )  # Typically takes 2-3 minutes, but hedging our bets
        ...
        async def guarded(start_index):
            async with semaphore:
                try:
                    ...
                    print(
                        f"⏳ Batch @ {start_index} timed out waiting for response – check DB for results"
                    )
